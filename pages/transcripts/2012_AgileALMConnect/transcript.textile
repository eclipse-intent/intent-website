...
But I think this sentence is often misunderstood : it just means that the best documentation in the world is no excuse if the project fails to deliver a working software. 
...
Agile Documents document must be able to respond to changes. In other words, we must be able to update it when the stakeholders needs or the software code change. It is very important that the few documentation we decided to create is accurate and up-to-date.
...
So we can see that if documentation is often discarded in Agile methods, it is because of its maintenance, which is both expensive and painful.
...
Once you defined custom validation rules on a project, ask yourself which ones are applicable to your next project. 
